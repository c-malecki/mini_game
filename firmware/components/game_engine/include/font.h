#ifndef __FONT_H_
#define __FONT_H_

#include <stdint.h>

/*
12px tall (7px wide per char),
a 64px-high display fits ~5 lines of text, 128px wide fits ~14 characters per line
*/

/* Ascii 7 x 12 font
 *
 * This is a pretty generic 7 x 12 ASCII font that includes
 * lower case descenders (indicated by (data & 0x80) != 0
 * on the first data byte of the character. Each row
 * represents 9 "lines" of the font, the bits in the byte
 * represent columns. When a glyph descends below the base
 * line it is rendered 3 pixels lower (hence the height of
 * 12 pixels rather than 9 even though the data is only 9
 * rows tall)
 */

#define FONT_7X12_WIDTH 7
#define FONT_7X12_HEIGHT 9
#define FONT_7X12_DESCENDER_OFFSET 3

extern const unsigned char font7x12[];

/*
 * 5 x 7 ASCII font.
 *
 * Each byte here has one "column" of the character
 * This minimizes the number of wasted bits (rather
 * than 7 bytes of 5 bit 'lines' it is 5 bytes of
 * 7 bit 'columns'.
 */

/* Generic to compute font bit on/off for this font */
#define FONT_BIT(c, column, row) ((font_data[(c) + (column)] & 1 << (7 - (row))) != 0)

static const unsigned char font_data[] = {
    
};

#endif // __FONT_H_